...
class model:
    def __init__(self,  file=None, nx=0, ny=0, ndep=0, nt=0, verb = True, dtype='float64'):
        self.dtype = dtype
        ...
        if(file != None):
            ...
            self.setSize(nx=nx, ny=ny, ndep=ndep, nt=nt, verb=verb)
    ...
    def extract(self, x0=0, x1=-1, y0=0, y1=-1, t0=0, t1=-1, z0=0, z1=-1, verb=True, dtype='float64'):
        if(x0 < 0): x0 = 0
        ...
        m = model(nx = nx, ny=ny, nt=nt, ndep=ndep, verb=verb, dtype=dtype)
        #
    ...
    def resize(self, nx=-1, ny=-1, ndep=-1):
        ...
        nm = model(nx=nx, ny=ny, nt=nt, ndep=ndep)
    ...
    def rebin(self, fac=1):
        ...
        m2 = model(nx = nx, ny = ny, ndep = self.ndep, nt = self.nt)
        for tt in range(self.nt):
...
class profile:
    ...
    def scale(self, ny=1, nx = 1):
        m = profile(nt=self.nt, ny=ny, nx=nx, nw=self.nw, ns=self.ns)
    ...
    def skip(self, fx=2, fy=2):
        ...
        m = profile(nx=nx, ny=ny, nt=nt, nw=nw, ns=ns)
        m.dat[:,:,:,:,:] = self.dat[:,0::fy,0::fx,:,:]
        m.cdat[:,:,:,:,:] = self.cdat[:,0::fy,0::fx,:,:]
        m.weights[:,:] = self.weights[:,:]
        m.wav[:] = self.wav
    ...
    def setsize(self, nx = 1, ny = 1, nw = 1, ns = 4, nt = 1):
        ...
        self.cdat = np.zeros((nt, ny, nx, nw, ns), order='c', dtype=self.dtype)
        ...
        self.psf = np.zeros((self.npy,self.npx), dtype='float64')
        self.rf = None

        self.lts = np.zeros((self.nlts,), dtype='int32')
        self.ltargs = np.zeros((self.nltargs,), dtype='float64')
        
        self.ds = np.zeros((self.ndsn,self.ndsy,self.ndsx), dtype='float64')
    ...
    def read(self, filename):
        ...
        for ii in nc_dims:
            ...
            elif(ii == 'lts'): self.nlts = len(nc_fid.dimensions['lts'])
            elif(ii == 'ltargs'): self.nltargs = len(nc_fid.dimensions['ltargs'])
            elif(ii == 'npx'): self.npx = len(nc_fid.dimensions['npx'])
            elif(ii == 'npy'): self.npy = len(nc_fid.dimensions['npy'])
            elif(ii == 'ndsn'): self.ndsn = len(nc_fid.dimensions['ndsn'])
            elif(ii == 'ndsx'): self.ndsx = len(nc_fid.dimensions['ndsx'])
            elif(ii == 'ndsy'): self.ndsy = len(nc_fid.dimensions['ndsy'])
        ...
        if(len(np.where(nc_vars == 'profiles')[0]) == 1):
            ...
            read += "[profiles]"
        if(len(np.where(nc_vars == 'cprofile')[0]) == 1):
            self.cdat[:,:,:,:,:] = nc_fid.variables['cprofile'][:]
            read += "[cprofile]"
        ...
        if(len(np.where(nc_vars == 'derivatives')[0]) == 1):
            ...
            read += '[vidx]'
        if(len(np.where(nc_vars == 'psf')[0]) == 1):
            self.psf[:,:] = nc_fid.variables['psf'][:]
            read += "[psf]"
        if(len(np.where(nc_vars == 'lts')[0]) == 1):
            self.lts = nc_fid.variables['lts'][:]
            read += "[lts]"
        if(len(np.where(nc_vars == 'ltargs')[0]) == 1):
            self.ltargs = nc_fid.variables['ltargs'][:]
            read += "[ltargs]"
        if(len(np.where(nc_vars == 'ds')[0]) == 1):
            self.ds[:,:,:] = nc_fid.variables['ds'][:]
            read += "[ds]"
    ...
    def write(self, filename):
        ...
        nc_fid.createDimension('lts',np.max([1,self.nlts]))
        nc_fid.createDimension('ltargs',np.max([1,self.nltargs]))
        nc_fid.createDimension('npx',self.npx)
        nc_fid.createDimension('npy',self.npy)

        nc_fid.createDimension('ndsn',self.ndsn)
        nc_fid.createDimension('ndsx',self.ndsx)
        nc_fid.createDimension('ndsy',self.ndsy)
        ...
        psf = nc_fid.createVariable('psf', 'f8' ,('npy', 'npx'))

        lts = nc_fid.createVariable('lts', 'i4' ,('lts', ))
        ltargs = nc_fid.createVariable('ltargs', 'f8' ,('ltargs', ))
        ds = nc_fid.createVariable('ds', 'f8' ,('ndsn', 'ndsy', 'ndsx'))
        ...
        if (self.nlts==0):
          lts[:]=np.array([-1],dtype='int32')
          ltargs[:]=np.array([-1.],dtype='float64')
        else:
          lts[:] = np.int32(self.lts)
          ltargs[:] = self.ltargs
    ...
    def __add__(self, o):
        ...
        n = profile(nx=self.nx, ny=self.ny, nw = nw, ns = self.ns, nt = self.nt)
        #
        # Check linear transformations:
        if(all(self.lts == o.lts) and\
           all(self.ltargs == o.ltargs)):
           n.lts = self.lts.copy()
           n.ltargs = self.ltargs.copy()
        else: return None
        #
    ...
    def extractWav(self, w0 = 0, w1 = -1):
        ...
        n.cdat[:,:,:,:,:] = self.cdat[:,:,:,w0:w1,:].copy()
        n.wav[:] = self.wav[w0:w1].copy()
        n.weights[:,:] = self.weights[w0:w1,:].copy()
        n.pweights[:,:,:] = self.pweights.copy()
        n.set_psf(psf=self.psf.copy())

        n.nlts = self.nlts
        n.lts = self.lts
        n.ltargs = self.ltargs
        n.nltargs = self.nltargs
    ...
    def extractPix(self, x0 = 0, x1 = -1, y0 = 0, y1 = -1, t0=0, t1=-1):
        ...
        n = profile(nx = nx, ny = ny, nt = nt, nw = self.nw)

        n.dat[:,:,:,:,:] = self.dat[t0:t1,y0:y1, x0:x1, :, :]
        #n.cdat[:,:,:,:,:] = self.cdat[t0:t1,y0:y1, x0:x1, :, :]
        n.wav[:] = self.wav
    ...
    def set_psf(self, sx=0, sy=0, x0=np.inf, y0=np.inf, psftype="gaussian", npx=-1, npy=-1, dx=1., dy=1., psf=None):

      if (type(psf)==type(None)):
  
        if (npy<0):
          y=np.arange(self.ny,dtype="f8")
        else:
          y=np.arange(npy,dtype="f8")
        self.npy=y.size
        if (y0 == np.inf):
          y0 = self.npy//2 # This is the reference taken inside STiC!
  
        if (npx<0):
          x=np.arange(self.nx,dtype="f8")
        else:
          x=np.arange(npx,dtype="f8")
        self.npx=x.size
        if (x0 == np.inf):
          x0 = self.npx//2 # This is the reference taken inside STiC!
  
        if (psftype.lower()=="gaussian"):
          self.psf = np.exp( - (y-y0)**2 / (2. * sy**2) )[:,None] \
              * np.exp( - (x-x0)**2 / (2. * sx**2))[None,:]
        elif (psftype.lower()=="lorentzian"):
        ###  gx = sx / np.sqrt(2. * np.log(2.))
        ###  gy = sy / np.sqrt(2. * np.log(2.))
        ###  gam=(gx+gy)/2.
  
          gam = (sx+sy) / 2.
          self.psf=1./2./np.pi*gam/np.sqrt((((x*dx)[None,:]-x0*dx)**2\
              +((y*dy)[:,None]-y0*dy)**2+gam**2)**3)
        ...
        dims = np.shape(psf)
        if (len(dims)!=2):
          raise Exception("If provided with a psf, it has to be rank 2!")
        self.npx = dims[1]
        self.npy = dims[0]
        self.psf = psf.copy()
    ...
    def add_linear_transformation(self, ilttype, ltargs):

      def printout_error(typ, inp, req):
        out = 1
        if (len(inp)!=req):
          print("\t%s: %i arguments are mandatory!" % (typ,req,))
          print("\t\t%i provided" % (len(inp),))
          out = np.nan
        return out

      if (type(ilttype)!=type([])):
        lttype = [ilttype,]
      else:
        lttype = ilttype.copy()

      # 1- Shift, 2- Rebin?, 3- Rotate, 4 Rebin+Trim, 10 de-stretching
      #      2         4           3          6
      if (lttype[0]==1):
        proc = printout_error("Shift", ltargs, 2)
      elif (lttype[0]==2):
        proc = printout_error("Binning", ltargs, 4)
      elif (lttype[0]==3):
        proc = printout_error("Rotation", ltargs, 3)
      elif (lttype[0]==4):
        proc = printout_error("Bin+Trim-ing", ltargs, 6)
      elif (lttype[0]==10):
        proc = printout_error("De-stretching", ltargs, 14)
      else:
        print("lttype = %s not allowed, it must be in [1-4|10]!" % (lttype,))
        return np.nan

      if (lttype[0]==10):

        tmpltargs = ltargs[0:12]
        self.ds = np.array(ltargs[12:])
        tmpnx, tmpny = np.int64(tmpltargs[6:8])
        _, dy, dx = self.ds.shape
        if ((dx!=0) & (dy!=0)):
          if ( (tmpny!=dy) | (tmpnx!=dx) ):
            print(" lttype = %i requires matching dimensions:" % (lttype[0]))
            print(" \theader: (%i,%i)" % (dy, dx))
            print(" \tde-stretching: (%i,%i)" % (tmpny, tmpnx))
            return np.nan
        self.ndsn = 2
        self.ndsx = dx
        self.ndsy = dy
        ...
        # Include a check!
        tmpnx, tmpny = np.int64(tmpltargs[0:2])
        if ((self.nx!=0) & (self.ny!=0)):
          if ( (tmpny!=self.ny) | (tmpnx!=self.nx) ):
            print(" lttype = %i requires matching dimensions:" % (lttype[0]))
            print(" \tprofiles: (%i,%i)" % (self.ny, self.nx))
            print(" \tde-stretching: (%i,%i)" % (tmpny, tmpnx))
            return np.nan

      else:

        tmpltargs = ltargs.copy()

      if (self.nlts==0):
        self.lts = np.append(self.lts, lttype)
        self.ltargs = np.append(self.ltargs, tmpltargs)
        self.nlts += 1
        self.nltargs += np.size(tmpltargs)
      elif (self.lts[0]!=-1):
        self.lts = np.append(self.lts, lttype)
        self.ltargs = np.append(self.ltargs, tmpltargs)
        self.nlts += 1
        ...
        self.lts=np.array(lttype,dtype='int32')
        self.ltargs=np.array(tmpltargs,dtype='float64')
        self.nltargs = np.size(tmpltargs)
      ...
      #  if (self.nlts==0):
      #    self.lts = np.append(self.lts, lttype)
      #    self.ltargs = np.append(self.ltargs, ltargs)
      #    self.nlts += 1
      #    self.nltargs += np.size(ltargs)
      #  elif (self.lts[0]!=-1):
      #    self.lts = np.append(self.lts, lttype)
      #    self.ltargs = np.append(self.ltargs, ltargs)
      #    self.nlts += 1
      ...
      #  else:
      #    self.lts=np.array(lttype,dtype='int32')
      #    self.ltargs=np.array(ltargs,dtype='float64')
      #    self.nltargs = np.size(ltargs)

      return


def writeInversionMask(oname, var):
    ncfile1 = nf(oname,'w', format='NETCDF4')
    ny, nx = var.shape
    ncfile1.createDimension('nx',nx)
    ncfile1.createDimension('ny',ny)
    par1 = ncfile1.createVariable('mask','i4',('ny','nx'))
    par1[:,:] = var
...
def writeInstProfMany(oname, var, pref=[]):

    if(var is None): return
  
    ny, nx, nwav = var.shape
    ncfile1 = nf(oname,'w', format='NETCDF4')
    ncfile1.createDimension('wav', nwav)
    ncfile1.createDimension( 'y', ny)
    ncfile1.createDimension( 'x', nx)
  
    par1 = ncfile1.createVariable('iprof','f8',('y','x','wav'))
    par1[:,:,:] = var
...
#
#
def get_init_model(nx=1,ny=1,nt=1,vmax=0.7, vmin=-6., vstp=0.1, axis='cmass', mag=False):

  from scipy.interpolate import interp1d

  itemp = np.float32([20000., 10000., 6000., 4000., 5000., 7000.])

  ndep = np.ceil(np.abs(vmin-vmax)/vstp).astype("i8") + 1
  vmax = vmin + ndep * vstp
  nvs = np.arange(ndep) * vstp + vmin

  m = model(nx=nx,ny=ny,ndep=ndep,nt=nt)

  if (axis=='cmass'):
    ivs = np.float32([-6.5, -5.2, -3.8, -1.5, -0.4, 1.5])
    m.ltau = m.ltau * 0. - 9.
    m.cmass = m.cmass * 0. + nvs[None,None,None,:]
  elif (axis=='ltau'):
    ivs = np.float32([-8.0, -6.8, -5.5, -4.0, -2.0, 1.0])
    m.ltau = m.ltau * 0. + nvs[None,None,None,:]

  fint = interp1d(ivs, itemp, fill_value="extrapolate")

  m.temp = m.temp* 0. + fint(nvs)[None,None,None,:]
  m.pgas = m.pgas * 0. + 0.3
  m.vlos = m.vlos * 0. + 1.e5
  m.vturb = m.vturb * 0. + 1.e5

  if (mag==True):
    m.Bln = m.Bln * 0. + 120
    m.Bho = m.Bho * 0. + 120
    m.azi = m.azi * 0. + np.pi/7.
